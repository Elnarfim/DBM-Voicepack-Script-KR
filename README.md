...
Interface - 와우 toc 버전입니다. 10.0.0은 100000 / 리분 클래식 30400 입니다. 패치가 나오면 toc 버전이 올라갈 수 있으니 주기적으로 업데이트 필요
...
X-DBM-Voice-Version - DBM 음성팩 버전 번호이며 DBM 업데이트에 따라 버전 넘버가 조금씩 올라갑니다. 용군단 사전 패치인 현재 12로 되어 있습니다. 보이스 버전 넘버는 앞으로 업데이트될 스크립트 첫줄에 명시되어 있으므로 주기적으로 이곳을 방문해서 확인하면 됩니다. 더 빨리 알고 싶다면 커스의 DBM Voicepack Demo 최신 알파 버전을 받아서 toc 파일을 열어보면 됩니다. <b>버전 번호가 DBM과 맞지 않으면 음성팩이 로딩되질 않으니 주의하세요</b>
