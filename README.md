...
Interface - 와우 toc 버전입니다. 본문 가장 위에 toc 버전이 표기되어 있습니다. 패치가 나오면 toc 버전이 올라갈 수 있으니 주기적으로 toc 버전 확인이 필요합니다. 로그인해서 /run GetBuildInfo() 명령어를 사용하면 확인 가능합니다. 10.2.7부터 클래식을 포함한 여러 버전의 클라이언트 toc 버전을 단일 toc 파일에 모두 넣을 수 있습니다. 100105, 40400, 10503 같은 형태로 콤마로 구분하면 됩니다.
